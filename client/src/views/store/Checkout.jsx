...
// Load PayPal script only once at component mount
const PayPalWrapper = ({ options, children }) => {
...
function Checkout() {
  ...
  const paypalOptions = {
    ...
    currency: "USD",
    intent: "capture",
    "data-namespace": "paypal_sdk", // Prevents namespace conflicts
    "data-react-version": React.version, // Ensures React version compatibility
  };

  const createPayPalOrder = async (data, actions) => {
    try {
      if (!order?.total) {
        throw new Error("Order total is not available");
      }
  ...
  const onApprovePayPalOrder = async (data, actions) => {
    try {
      ...
      if (captureResult.status === "COMPLETED") {
        navigate(
          `/payment-success/${order.oid}/?paypal_order_id=${captureResult.id}`
        );
      }
  ...
  const onErrorHandler = (error) => {
    ...
    Toast.fire({
      icon: "error",
      title: "Payment failed. Please try another method.",
    });
  };
  ...
  const fetchOrderData = () => {
    apiInstance.get(`checkout/${param.order_oid}/`).then((res) => {
      ...
      setPaypalLoaded(true); // Only load PayPal after order data is available
    }).catch(error => {
          ...
          <div className="col-lg-4">
            ...
            >
              <div className="card-body p-4">
                ...
                <div className="mb-4">
                  {[
                    { label: "Subtotal", value: order?.sub_total || "0.00" },
                    {
                      label: "Shipping",
                      value: order?.shipping_amount || "0.00",
                    },
                    { label: "Tax", value: order?.tax_fee || "0.00" },
                    {
                      label: "Service Fee",
                      value: order?.service_fee || "0.00",
                    },
                    {
                      label: "Savings",
                      value: order?.saved || "0.00",
                      isNegative: true,
                    },
                  ].map((item, index) => (
                    <div
                ...
                <div className="mb-4">
                  <div className="row g-2">
                    ...
                    <div className="col-12 mt-3">
                      {paypalLoaded && order?.total > 0 && (
                        <PayPalWrapper options={paypalOptions}>
                          <PayPalButtons
                            style={{ layout: "vertical" }}
                            createOrder={createPayPalOrder}
                            onApprove={onApprovePayPalOrder}
                            onError={onErrorHandler}
                            forceReRender={[order?.total]}
                          />
                        </PayPalWrapper>
                      )}
                    </div>
